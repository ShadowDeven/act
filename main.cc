...
int convert_string_to_elem(string& line, COVG_MAP_VEC& trace, Config_Map& map_config, vector<struct Test_Parems>& test_para_vec)
{
	...
	index_i++;
C
	if (cwnd > 0 && cwnd <= WND_RANGE && ssthresh > 0 && ssthresh <= SSTH_RANGE && srtt > 0 && srtt <= RTT_RANGE && rttvar > 0 && rttvar <= RTVAR_RANGE && state >= 0 && state < STATE_RANGE && curr_time > 0 && target > 0 && target <= TARGET_RANGE)  //ssthresh at least 2
	{rttvar
		struct State_Record tmp(cwnd, ssthresh, srtt, , state, target, curr_time);
...
int try_per_config(int i, int mode) //test input i
{
	...
	{
		res = execute_ns3_2(0); // run ns3 in mode 0
	}
	
	if (res == -2)	// execute ns3 fails
	{
...
int init_coverage_map_vec()
{
	...
	for (int i = 0; i < 10; i ++) // Now we have coverage with size of 1, 2, 4, 8, 16, 32 ...... 512
	{
		...
		granularity = pow(2, i);
		if (DEBUG) cout << "coverage map initialization, granularity:" << granularity  << endl;
...
int main (int argc, char* argv[])
{
	...
	cmd_init_random();	//initialise tmp/output_all directory
	...
	cal_coverage_AllGrans(covg_map_vec);	//print coverage information
